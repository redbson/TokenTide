...
test("formats reset times relative to now", () => {
  ...
  assert.equal(formatReset(inMinutes(-1), NOW), "即将重置");
  assert.equal(formatReset(inMinutes(42), NOW), "42 分钟后重置");
  assert.equal(formatReset(inMinutes(125), NOW), "2 小时 05 分后重置");
  assert.match(formatReset(inMinutes(3 * 24 * 60), NOW), /^\d+月\d+日 \d{2}:\d{2} 重置$/);
  assert.equal(formatReset({ resetLabel: "Sep 15 at 4pm (Europe/Berlin)" }, NOW), "Sep 15 at 4pm 重置");
  assert.equal(formatReset({}, NOW), "重置时间未知");
});

test("formats last-updated time", () => {
  assert.equal(timeAgo(null, NOW), "尚未更新");
  assert.equal(timeAgo(NOW - 5_000, NOW), "刚刚更新");
  assert.equal(timeAgo(NOW - 30_000, NOW), "30 秒前更新");
  assert.equal(timeAgo(NOW - 5 * 60_000, NOW), "5 分钟前更新");
  assert.equal(timeAgo(NOW - 2 * 3_600_000, NOW), "2 小时前更新");
});
...
test("describes low providers", () => {
  assert.equal(
    describeLow([provider("codex", 0), provider("claude", 12.4)]),
    "Codex 当前窗口额度已用尽，Claude Code 仅剩 12%",
  );
});

test("localizes account labels", () => {
  assert.equal(formatAccount("team plan"), "Team 套餐");
  assert.equal(formatAccount("pro lite plan"), "Pro Lite 套餐");
  assert.equal(formatAccount("max plan"), "Max 套餐");
  assert.equal(formatAccount("Usage view fallback"), "/usage 备用读取");
  assert.equal(formatAccount("Local account"), "本机账户");
  assert.equal(formatAccount(null), null);
});
